"""
Internal Carbon Pricing (ICP) & Shadow Carbon Tax Investment Filter
...
class CarbonOffsetPricingShadowTaxReportingInput(BaseModel):
...
class CarbonOffsetPricingShadowTaxEmissionsPillar(BaseModel):
    pillar_name: str
    emissions_metric_tonnes_co2e: float
    percentage_of_total: float
    decarbonization_target_2030: float

class CarbonOffsetPricingShadowTaxSustainabilityReport(BaseModel):
    disclosure_framework: str = "Internal Carbon Pricing (ICP) & Shadow Carbon Tax Investment Filter"
    ...
    pillars: List[CarbonOffsetPricingShadowTaxEmissionsPillar]
    decarbonization_milestones: List[str]

class CarbonOffsetPricingShadowTaxEngine:
    @classmethod
    def generate_sustainability_disclosure(
        cls, inp: CarbonOffsetPricingShadowTaxReportingInput
    ) -> CarbonOffsetPricingShadowTaxSustainabilityReport:
        ...
        pillars = [
            CarbonOffsetPricingShadowTaxEmissionsPillar(
        ...
        return CarbonOffsetPricingShadowTaxSustainabilityReport(
            disclosure_framework="Internal Carbon Pricing (ICP) & Shadow Carbon Tax Investment Filter",
