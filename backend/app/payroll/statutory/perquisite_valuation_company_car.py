"""
Rule 3 Company Car, Driver & Fuel Perquisite Tax Valuator
...
class PerquisiteValuationCompanyCarRequest(BaseModel):
...
class PerquisiteValuationCompanyCarBreakdown(BaseModel):
    component_code: str
    component_name: str
...
class PerquisiteValuationCompanyCarResult(BaseModel):
    policy_name: str = "Rule 3 Company Car, Driver & Fuel Perquisite Tax Valuator"
    employee_id: str
    total_employee_deduction_monthly: float
    total_employer_contribution_monthly: float
    net_take_home_impact_monthly: float
    breakdown_items: List[PerquisiteValuationCompanyCarBreakdown]
    compliance_advisory: List[str]

class PerquisiteValuationCompanyCarEngine:
    @classmethod
    def calculate_statutory_benefit(cls, req: PerquisiteValuationCompanyCarRequest) -> PerquisiteValuationCompanyCarResult:
        ...
        items = [
            PerquisiteValuationCompanyCarBreakdown(
        ...
        return PerquisiteValuationCompanyCarResult(
            policy_name="Rule 3 Company Car, Driver & Fuel Perquisite Tax Valuator",
