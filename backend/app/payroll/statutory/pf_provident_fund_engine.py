"""
Employees Provident Fund (EPF) 12% + 12% and EPS Split Calculator
...
class PfProvidentFundEngineRequest(BaseModel):
...
class PfProvidentFundEngineBreakdown(BaseModel):
    component_code: str
    component_name: str
...
class PfProvidentFundEngineResult(BaseModel):
    policy_name: str = "Employees Provident Fund (EPF) 12% + 12% and EPS Split Calculator"
    employee_id: str
    total_employee_deduction_monthly: float
    total_employer_contribution_monthly: float
    net_take_home_impact_monthly: float
    breakdown_items: List[PfProvidentFundEngineBreakdown]
    compliance_advisory: List[str]

class PfProvidentFundEngineEngine:
    @classmethod
    def calculate_statutory_benefit(cls, req: PfProvidentFundEngineRequest) -> PfProvidentFundEngineResult:
        ...
        items = [
            PfProvidentFundEngineBreakdown(
        ...
        return PfProvidentFundEngineResult(
            policy_name="Employees Provident Fund (EPF) 12% + 12% and EPS Split Calculator",
