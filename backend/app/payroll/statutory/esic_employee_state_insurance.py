"""
ESIC 0.75% Employee + 3.25% Employer Statutory Wage Pool
...
class EsicEmployeeStateInsuranceRequest(BaseModel):
...
class EsicEmployeeStateInsuranceBreakdown(BaseModel):
    component_code: str
    component_name: str
...
class EsicEmployeeStateInsuranceResult(BaseModel):
    policy_name: str = "ESIC 0.75% Employee + 3.25% Employer Statutory Wage Pool"
    employee_id: str
    total_employee_deduction_monthly: float
    total_employer_contribution_monthly: float
    net_take_home_impact_monthly: float
    breakdown_items: List[EsicEmployeeStateInsuranceBreakdown]
    compliance_advisory: List[str]

class EsicEmployeeStateInsuranceEngine:
    @classmethod
    def calculate_statutory_benefit(cls, req: EsicEmployeeStateInsuranceRequest) -> EsicEmployeeStateInsuranceResult:
        ...
        items = [
            EsicEmployeeStateInsuranceBreakdown(
        ...
        return EsicEmployeeStateInsuranceResult(
            policy_name="ESIC 0.75% Employee + 3.25% Employer Statutory Wage Pool",
