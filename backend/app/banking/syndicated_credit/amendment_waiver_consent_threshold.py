"""
LMA Required Lenders (66.67% / 100%) Amendment Consent Engine
...
class AmendmentWaiverConsentThresholdSyndicateStructure(BaseModel):
...
class AmendmentWaiverConsentThresholdParticipantLender(BaseModel):
    lender_bic: str
    lender_name: str
...
class AmendmentWaiverConsentThresholdFacilitySummary(BaseModel):
    facility_name: str = "LMA Required Lenders (66.67% / 100%) Amendment Consent Engine"
    ...
    lenders_syndicate: List[AmendmentWaiverConsentThresholdParticipantLender]
    governance_covenants: List[str]

class AmendmentWaiverConsentThresholdEngine:
    @classmethod
    def manage_syndicate(
        cls, s: AmendmentWaiverConsentThresholdSyndicateStructure
    ) -> AmendmentWaiverConsentThresholdFacilitySummary:
        ...
        lenders: List[AmendmentWaiverConsentThresholdParticipantLender] = []
        for name, pct in banks_config:
            comm = tot_vol * pct
            int_mo = monthly_interest * pct
            lenders.append(AmendmentWaiverConsentThresholdParticipantLender(
        ...
        return AmendmentWaiverConsentThresholdFacilitySummary(
