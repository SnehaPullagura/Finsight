"""
Voluntary Prepayment Notice & Funding Breakage Cost Calculator
...
class PrepaymentBreakageCostCalculatorSyndicateStructure(BaseModel):
...
class PrepaymentBreakageCostCalculatorParticipantLender(BaseModel):
    lender_bic: str
    lender_name: str
...
class PrepaymentBreakageCostCalculatorFacilitySummary(BaseModel):
    facility_name: str = "Voluntary Prepayment Notice & Funding Breakage Cost Calculator"
    ...
    lenders_syndicate: List[PrepaymentBreakageCostCalculatorParticipantLender]
    governance_covenants: List[str]

class PrepaymentBreakageCostCalculatorEngine:
    @classmethod
    def manage_syndicate(
        cls, s: PrepaymentBreakageCostCalculatorSyndicateStructure
    ) -> PrepaymentBreakageCostCalculatorFacilitySummary:
        ...
        lenders: List[PrepaymentBreakageCostCalculatorParticipantLender] = []
        for name, pct in banks_config:
            comm = tot_vol * pct
            int_mo = monthly_interest * pct
            lenders.append(PrepaymentBreakageCostCalculatorParticipantLender(
        ...
        return PrepaymentBreakageCostCalculatorFacilitySummary(
