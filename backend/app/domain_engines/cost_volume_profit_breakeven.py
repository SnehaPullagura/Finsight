"""
Contribution Margin, Operating Leverage & Breakeven Visualizer
Module Index: 31 of 50
...
class CostVolumeProfitBreakevenInputParameters(BaseModel):
...
class CostVolumeProfitBreakevenPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class CostVolumeProfitBreakevenEngineResult(BaseModel):
    engine_name: str = "Contribution Margin, Operating Leverage & Breakeven Visualizer"
    ...
    detailed_period_waterfall: List[CostVolumeProfitBreakevenPeriodScheduleItem]
    optimization_recommendations: List[str]

class CostVolumeProfitBreakevenEngine:
    """
    Institutional Implementation of Contribution Margin, Operating Leverage & Breakeven Visualizer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: CostVolumeProfitBreakevenInputParameters) -> CostVolumeProfitBreakevenEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[CostVolumeProfitBreakevenPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(CostVolumeProfitBreakevenPeriodScheduleItem(
        ...
        return CostVolumeProfitBreakevenEngineResult(
