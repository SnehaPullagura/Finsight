"""
Activity-Based Costing (ABC) Overhead Pool Allocation Engine
Module Index: 32 of 50
...
class ActivityBasedCostingAllocatorInputParameters(BaseModel):
...
class ActivityBasedCostingAllocatorPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class ActivityBasedCostingAllocatorEngineResult(BaseModel):
    engine_name: str = "Activity-Based Costing (ABC) Overhead Pool Allocation Engine"
    ...
    detailed_period_waterfall: List[ActivityBasedCostingAllocatorPeriodScheduleItem]
    optimization_recommendations: List[str]

class ActivityBasedCostingAllocatorEngine:
    """
    Institutional Implementation of Activity-Based Costing (ABC) Overhead Pool Allocation Engine.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: ActivityBasedCostingAllocatorInputParameters) -> ActivityBasedCostingAllocatorEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[ActivityBasedCostingAllocatorPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(ActivityBasedCostingAllocatorPeriodScheduleItem(
        ...
        return ActivityBasedCostingAllocatorEngineResult(
