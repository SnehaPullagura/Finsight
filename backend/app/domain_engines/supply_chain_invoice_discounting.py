"""
TReDS / Supply Chain Reverse Factoring Discounting Calculator
Module Index: 08 of 50
...
class SupplyChainInvoiceDiscountingInputParameters(BaseModel):
...
class SupplyChainInvoiceDiscountingPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class SupplyChainInvoiceDiscountingEngineResult(BaseModel):
    engine_name: str = "TReDS / Supply Chain Reverse Factoring Discounting Calculator"
    ...
    detailed_period_waterfall: List[SupplyChainInvoiceDiscountingPeriodScheduleItem]
    optimization_recommendations: List[str]

class SupplyChainInvoiceDiscountingEngine:
    """
    Institutional Implementation of TReDS / Supply Chain Reverse Factoring Discounting Calculator.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: SupplyChainInvoiceDiscountingInputParameters) -> SupplyChainInvoiceDiscountingEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[SupplyChainInvoiceDiscountingPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(SupplyChainInvoiceDiscountingPeriodScheduleItem(
        ...
        return SupplyChainInvoiceDiscountingEngineResult(
