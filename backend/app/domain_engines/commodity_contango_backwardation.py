"""
Futures Roll Yield & Term Structure Contango / Backwardation
Module Index: 48 of 50
...
class CommodityContangoBackwardationInputParameters(BaseModel):
...
class CommodityContangoBackwardationPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class CommodityContangoBackwardationEngineResult(BaseModel):
    engine_name: str = "Futures Roll Yield & Term Structure Contango / Backwardation"
    ...
    detailed_period_waterfall: List[CommodityContangoBackwardationPeriodScheduleItem]
    optimization_recommendations: List[str]

class CommodityContangoBackwardationEngine:
    """
    Institutional Implementation of Futures Roll Yield & Term Structure Contango / Backwardation.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: CommodityContangoBackwardationInputParameters) -> CommodityContangoBackwardationEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[CommodityContangoBackwardationPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(CommodityContangoBackwardationPeriodScheduleItem(
        ...
        return CommodityContangoBackwardationEngineResult(
