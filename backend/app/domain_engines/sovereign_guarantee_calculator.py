...
class SovereignGuaranteeCalculatorInputParameters(BaseModel):
...
class SovereignGuaranteeCalculatorPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class SovereignGuaranteeCalculatorEngineResult(BaseModel):
    engine_name: str = "Credit Guarantee Fund Trust for Micro & Small Enterprises (CGTMSE)"
    ...
    detailed_period_waterfall: List[SovereignGuaranteeCalculatorPeriodScheduleItem]
    optimization_recommendations: List[str]

class SovereignGuaranteeCalculatorEngine:
    """
    Institutional Implementation of Credit Guarantee Fund Trust for Micro & Small Enterprises (CGTMSE).
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: SovereignGuaranteeCalculatorInputParameters) -> SovereignGuaranteeCalculatorEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[SovereignGuaranteeCalculatorPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(SovereignGuaranteeCalculatorPeriodScheduleItem(
        ...
        return SovereignGuaranteeCalculatorEngineResult(
