"""
Employee Stock Option Plan (ESOP) Perquisite Tax & FMV Estimator
Module Index: 28 of 50
...
class EsopExerciseTaxPerquisiteInputParameters(BaseModel):
...
class EsopExerciseTaxPerquisitePeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class EsopExerciseTaxPerquisiteEngineResult(BaseModel):
    engine_name: str = "Employee Stock Option Plan (ESOP) Perquisite Tax & FMV Estimator"
    ...
    detailed_period_waterfall: List[EsopExerciseTaxPerquisitePeriodScheduleItem]
    optimization_recommendations: List[str]

class EsopExerciseTaxPerquisiteEngine:
    """
    Institutional Implementation of Employee Stock Option Plan (ESOP) Perquisite Tax & FMV Estimator.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: EsopExerciseTaxPerquisiteInputParameters) -> EsopExerciseTaxPerquisiteEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[EsopExerciseTaxPerquisitePeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(EsopExerciseTaxPerquisitePeriodScheduleItem(
        ...
        return EsopExerciseTaxPerquisiteEngineResult(
