"""
Single-Name & Index Credit Default Swap (CDS) Spread Pricer
Module Index: 49 of 50
...
class HighYieldCreditDefaultSwapInputParameters(BaseModel):
...
class HighYieldCreditDefaultSwapPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class HighYieldCreditDefaultSwapEngineResult(BaseModel):
    engine_name: str = "Single-Name & Index Credit Default Swap (CDS) Spread Pricer"
    ...
    detailed_period_waterfall: List[HighYieldCreditDefaultSwapPeriodScheduleItem]
    optimization_recommendations: List[str]

class HighYieldCreditDefaultSwapEngine:
    """
    Institutional Implementation of Single-Name & Index Credit Default Swap (CDS) Spread Pricer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: HighYieldCreditDefaultSwapInputParameters) -> HighYieldCreditDefaultSwapEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[HighYieldCreditDefaultSwapPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(HighYieldCreditDefaultSwapPeriodScheduleItem(
        ...
        return HighYieldCreditDefaultSwapEngineResult(
