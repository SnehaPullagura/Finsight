"""
P2P Credit Risk Tiering & Fractional Loan Basket Allocator
Module Index: 13 of 50
...
class PeerToPeerLendingAllocatorInputParameters(BaseModel):
...
class PeerToPeerLendingAllocatorPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class PeerToPeerLendingAllocatorEngineResult(BaseModel):
    engine_name: str = "P2P Credit Risk Tiering & Fractional Loan Basket Allocator"
    ...
    detailed_period_waterfall: List[PeerToPeerLendingAllocatorPeriodScheduleItem]
    optimization_recommendations: List[str]

class PeerToPeerLendingAllocatorEngine:
    """
    Institutional Implementation of P2P Credit Risk Tiering & Fractional Loan Basket Allocator.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: PeerToPeerLendingAllocatorInputParameters) -> PeerToPeerLendingAllocatorEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[PeerToPeerLendingAllocatorPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(PeerToPeerLendingAllocatorPeriodScheduleItem(
        ...
        return PeerToPeerLendingAllocatorEngineResult(
