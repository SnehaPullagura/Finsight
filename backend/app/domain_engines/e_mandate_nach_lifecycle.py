"""
NPCI e-NACH & UPI AutoPay Subscription Mandate Lifecycle Engine
Module Index: 04 of 50
...
class EMandateNachLifecycleInputParameters(BaseModel):
...
class EMandateNachLifecyclePeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class EMandateNachLifecycleEngineResult(BaseModel):
    engine_name: str = "NPCI e-NACH & UPI AutoPay Subscription Mandate Lifecycle Engine"
    ...
    detailed_period_waterfall: List[EMandateNachLifecyclePeriodScheduleItem]
    optimization_recommendations: List[str]

class EMandateNachLifecycleEngine:
    """
    Institutional Implementation of NPCI e-NACH & UPI AutoPay Subscription Mandate Lifecycle Engine.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: EMandateNachLifecycleInputParameters) -> EMandateNachLifecycleEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[EMandateNachLifecyclePeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(EMandateNachLifecyclePeriodScheduleItem(
        ...
        return EMandateNachLifecycleEngineResult(
