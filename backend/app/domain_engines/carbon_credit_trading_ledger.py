"""
Voluntary Carbon Market (VCM) Credit Offsetting & Valuation Engine
Module Index: 30 of 50
...
class CarbonCreditTradingLedgerInputParameters(BaseModel):
...
class CarbonCreditTradingLedgerPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class CarbonCreditTradingLedgerEngineResult(BaseModel):
    engine_name: str = "Voluntary Carbon Market (VCM) Credit Offsetting & Valuation Engine"
    ...
    detailed_period_waterfall: List[CarbonCreditTradingLedgerPeriodScheduleItem]
    optimization_recommendations: List[str]

class CarbonCreditTradingLedgerEngine:
    """
    Institutional Implementation of Voluntary Carbon Market (VCM) Credit Offsetting & Valuation Engine.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: CarbonCreditTradingLedgerInputParameters) -> CarbonCreditTradingLedgerEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[CarbonCreditTradingLedgerPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(CarbonCreditTradingLedgerPeriodScheduleItem(
        ...
        return CarbonCreditTradingLedgerEngineResult(
