"""
Key Rate Duration (KRD) & Non-Parallel Yield Curve Shift Vector
...
class BondPortfolioKeyRateDurationInstrumentParameters(BaseModel):
...
class BondPortfolioKeyRateDurationCashFlowPoint(BaseModel):
    period_number: int
    due_date: str
...
class BondPortfolioKeyRateDurationValuationResult(BaseModel):
    instrument_title: str = "Key Rate Duration (KRD) & Non-Parallel Yield Curve Shift Vector"
    ...
    cash_flow_timeline: List[BondPortfolioKeyRateDurationCashFlowPoint]
    risk_sensitivities: Dict[str, float]

class BondPortfolioKeyRateDurationEngine:
    @classmethod
    def calculate_valuation(
        cls, params: BondPortfolioKeyRateDurationInstrumentParameters
    ) -> BondPortfolioKeyRateDurationValuationResult:
        ...
        timeline: List[BondPortfolioKeyRateDurationCashFlowPoint] = []
        ...
        for t in range(1, n_periods + 1):
            ...
            timeline.append(BondPortfolioKeyRateDurationCashFlowPoint(
                period_number=t,
        ...
        return BondPortfolioKeyRateDurationValuationResult(
