"""
Dynamic Working Capital & Days Sales Outstanding (DSO) Optimizer
Module Index: 03 of 50
...
class WorkingCapitalOptimizationInputParameters(BaseModel):
...
class WorkingCapitalOptimizationPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class WorkingCapitalOptimizationEngineResult(BaseModel):
    engine_name: str = "Dynamic Working Capital & Days Sales Outstanding (DSO) Optimizer"
    ...
    detailed_period_waterfall: List[WorkingCapitalOptimizationPeriodScheduleItem]
    optimization_recommendations: List[str]

class WorkingCapitalOptimizationEngine:
    """
    Institutional Implementation of Dynamic Working Capital & Days Sales Outstanding (DSO) Optimizer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: WorkingCapitalOptimizationInputParameters) -> WorkingCapitalOptimizationEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[WorkingCapitalOptimizationPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(WorkingCapitalOptimizationPeriodScheduleItem(
        ...
        return WorkingCapitalOptimizationEngineResult(
