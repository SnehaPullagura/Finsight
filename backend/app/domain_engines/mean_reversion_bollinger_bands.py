...
class MeanReversionBollingerBandsInputParameters(BaseModel):
...
class MeanReversionBollingerBandsPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class MeanReversionBollingerBandsEngineResult(BaseModel):
    engine_name: str = "Bollinger Bands & Keltner Channel Volatility Squeeze Detector"
    ...
    detailed_period_waterfall: List[MeanReversionBollingerBandsPeriodScheduleItem]
    optimization_recommendations: List[str]

class MeanReversionBollingerBandsEngine:
    """
    Institutional Implementation of Bollinger Bands & Keltner Channel Volatility Squeeze Detector.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: MeanReversionBollingerBandsInputParameters) -> MeanReversionBollingerBandsEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[MeanReversionBollingerBandsPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(MeanReversionBollingerBandsPeriodScheduleItem(
        ...
        return MeanReversionBollingerBandsEngineResult(
