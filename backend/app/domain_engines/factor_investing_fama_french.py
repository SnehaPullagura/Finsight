"""
Fama-French 5-Factor & Momentum Equity Return Decomposer
Module Index: 21 of 50
...
class FactorInvestingFamaFrenchInputParameters(BaseModel):
...
class FactorInvestingFamaFrenchPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class FactorInvestingFamaFrenchEngineResult(BaseModel):
    engine_name: str = "Fama-French 5-Factor & Momentum Equity Return Decomposer"
    ...
    detailed_period_waterfall: List[FactorInvestingFamaFrenchPeriodScheduleItem]
    optimization_recommendations: List[str]

class FactorInvestingFamaFrenchEngine:
    """
    Institutional Implementation of Fama-French 5-Factor & Momentum Equity Return Decomposer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: FactorInvestingFamaFrenchInputParameters) -> FactorInvestingFamaFrenchEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[FactorInvestingFamaFrenchPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(FactorInvestingFamaFrenchPeriodScheduleItem(
        ...
        return FactorInvestingFamaFrenchEngineResult(
