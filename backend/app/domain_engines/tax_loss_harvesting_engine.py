"""
Wash-Sale Aware Capital Gains Tax Loss Harvesting Optimizer
Module Index: 24 of 50
...
class TaxLossHarvestingEngineInputParameters(BaseModel):
...
class TaxLossHarvestingEnginePeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class TaxLossHarvestingEngineEngineResult(BaseModel):
    engine_name: str = "Wash-Sale Aware Capital Gains Tax Loss Harvesting Optimizer"
    ...
    detailed_period_waterfall: List[TaxLossHarvestingEnginePeriodScheduleItem]
    optimization_recommendations: List[str]

class TaxLossHarvestingEngineEngine:
    """
    Institutional Implementation of Wash-Sale Aware Capital Gains Tax Loss Harvesting Optimizer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: TaxLossHarvestingEngineInputParameters) -> TaxLossHarvestingEngineEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[TaxLossHarvestingEnginePeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(TaxLossHarvestingEnginePeriodScheduleItem(
        ...
        return TaxLossHarvestingEngineEngineResult(
