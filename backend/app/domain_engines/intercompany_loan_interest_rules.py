"""
Safe Harbour Intercompany Lending Interest & Thin Capitalization
Module Index: 37 of 50
...
class IntercompanyLoanInterestRulesInputParameters(BaseModel):
...
class IntercompanyLoanInterestRulesPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class IntercompanyLoanInterestRulesEngineResult(BaseModel):
    engine_name: str = "Safe Harbour Intercompany Lending Interest & Thin Capitalization"
    ...
    detailed_period_waterfall: List[IntercompanyLoanInterestRulesPeriodScheduleItem]
    optimization_recommendations: List[str]

class IntercompanyLoanInterestRulesEngine:
    """
    Institutional Implementation of Safe Harbour Intercompany Lending Interest & Thin Capitalization.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: IntercompanyLoanInterestRulesInputParameters) -> IntercompanyLoanInterestRulesEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[IntercompanyLoanInterestRulesPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(IntercompanyLoanInterestRulesPeriodScheduleItem(
        ...
        return IntercompanyLoanInterestRulesEngineResult(
