"""
SGB Capital Gains Exemption & 2.5% Semi-Annual Interest Tracker
Module Index: 27 of 50
...
class SovereignGoldBondTranchesInputParameters(BaseModel):
...
class SovereignGoldBondTranchesPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class SovereignGoldBondTranchesEngineResult(BaseModel):
    engine_name: str = "SGB Capital Gains Exemption & 2.5% Semi-Annual Interest Tracker"
    ...
    detailed_period_waterfall: List[SovereignGoldBondTranchesPeriodScheduleItem]
    optimization_recommendations: List[str]

class SovereignGoldBondTranchesEngine:
    """
    Institutional Implementation of SGB Capital Gains Exemption & 2.5% Semi-Annual Interest Tracker.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: SovereignGoldBondTranchesInputParameters) -> SovereignGoldBondTranchesEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[SovereignGoldBondTranchesPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(SovereignGoldBondTranchesPeriodScheduleItem(
        ...
        return SovereignGoldBondTranchesEngineResult(
