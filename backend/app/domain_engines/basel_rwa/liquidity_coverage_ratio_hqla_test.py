"""
High-Quality Liquid Assets (HQLA) & 30-Day Net Cash Outflow LCR Engine
...
class LiquidityCoverageRatioHqlaTestBankPortfolio(BaseModel):
...
class LiquidityCoverageRatioHqlaTestAssetClassRWA(BaseModel):
    asset_class_name: str
    gross_exposure_amount: float
    applicable_risk_weight_pct: float
    calculated_rwa_amount: float

class LiquidityCoverageRatioHqlaTestCapitalAdequacyResult(BaseModel):
    regulatory_framework: str = "High-Quality Liquid Assets (HQLA) & 30-Day Net Cash Outflow LCR Engine"
    ...
    asset_class_rwa_breakdown: List[LiquidityCoverageRatioHqlaTestAssetClassRWA]
    supervisory_recommendations: List[str]

class LiquidityCoverageRatioHqlaTestEngine:
    @classmethod
    def evaluate_capital_adequacy(
        cls, p: LiquidityCoverageRatioHqlaTestBankPortfolio
    ) -> LiquidityCoverageRatioHqlaTestCapitalAdequacyResult:
        ...
        rwa_items = [
            LiquidityCoverageRatioHqlaTestAssetClassRWA(
        ...
        return LiquidityCoverageRatioHqlaTestCapitalAdequacyResult(
            regulatory_framework="High-Quality Liquid Assets (HQLA) & 30-Day Net Cash Outflow LCR Engine",
