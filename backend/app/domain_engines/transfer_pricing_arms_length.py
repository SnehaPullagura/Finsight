"""
OECD Arm's Length Principle Comparable Uncontrolled Price (CUP)
Module Index: 36 of 50
...
class TransferPricingArmsLengthInputParameters(BaseModel):
...
class TransferPricingArmsLengthPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class TransferPricingArmsLengthEngineResult(BaseModel):
    engine_name: str = "OECD Arm's Length Principle Comparable Uncontrolled Price (CUP)"
    ...
    detailed_period_waterfall: List[TransferPricingArmsLengthPeriodScheduleItem]
    optimization_recommendations: List[str]

class TransferPricingArmsLengthEngine:
    """
    Institutional Implementation of OECD Arm's Length Principle Comparable Uncontrolled Price (CUP).
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: TransferPricingArmsLengthInputParameters) -> TransferPricingArmsLengthEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[TransferPricingArmsLengthPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(TransferPricingArmsLengthPeriodScheduleItem(
        ...
        return TransferPricingArmsLengthEngineResult(
