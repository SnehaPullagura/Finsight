"""
Automated Two-Way Overdraft Sweep & Surplus Yield Maximizer
Module Index: 09 of 50
...
class OverdraftSweepingAlgorithmInputParameters(BaseModel):
...
class OverdraftSweepingAlgorithmPeriodScheduleItem(BaseModel):
    period_sequence: int
    period_label: str
...
class OverdraftSweepingAlgorithmEngineResult(BaseModel):
    engine_name: str = "Automated Two-Way Overdraft Sweep & Surplus Yield Maximizer"
    ...
    detailed_period_waterfall: List[OverdraftSweepingAlgorithmPeriodScheduleItem]
    optimization_recommendations: List[str]

class OverdraftSweepingAlgorithmEngine:
    """
    Institutional Implementation of Automated Two-Way Overdraft Sweep & Surplus Yield Maximizer.
    Provides quantitative modeling, regulatory compliance, stress testing, and cash-flow waterfall calculations.
    """
    @classmethod
    def execute_analysis(cls, params: OverdraftSweepingAlgorithmInputParameters) -> OverdraftSweepingAlgorithmEngineResult:
        r_step = (params.annual_operational_rate_pct / 100.0) / 12.0
        curr_val = params.primary_capital_value
        today = datetime.date.today()
        
        waterfall: List[OverdraftSweepingAlgorithmPeriodScheduleItem] = []
        tot_inflows = 0.0
        tot_outflows = 0.0
        ...
        for step in range(1, params.simulation_duration_periods + 1):
            ...
            waterfall.append(OverdraftSweepingAlgorithmPeriodScheduleItem(
        ...
        return OverdraftSweepingAlgorithmEngineResult(
