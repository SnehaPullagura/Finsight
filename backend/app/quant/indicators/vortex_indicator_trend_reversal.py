"""
Vortex Indicator (VI+ and VI- Positive/Negative Flow)
...
class VortexIndicatorTrendReversalInputData(BaseModel):
...
class VortexIndicatorTrendReversalIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class VortexIndicatorTrendReversalResult(BaseModel):
    indicator_title: str = "Vortex Indicator (VI+ and VI- Positive/Negative Flow)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[VortexIndicatorTrendReversalIndicatorPoint]

class VortexIndicatorTrendReversalEngine:
    @classmethod
    def calculate(cls, data: VortexIndicatorTrendReversalInputData) -> VortexIndicatorTrendReversalResult:
        ...
        series: List[VortexIndicatorTrendReversalIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(VortexIndicatorTrendReversalIndicatorPoint(
                index=i,
        ...
        return VortexIndicatorTrendReversalResult(
            current_value=curr,
