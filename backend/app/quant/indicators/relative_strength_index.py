...
class RelativeStrengthIndexInputData(BaseModel):
...
class RelativeStrengthIndexIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class RelativeStrengthIndexResult(BaseModel):
    indicator_title: str = "Relative Strength Index (RSI) with Wilder Smoothing"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[RelativeStrengthIndexIndicatorPoint]

class RelativeStrengthIndexEngine:
    @classmethod
    def calculate(cls, data: RelativeStrengthIndexInputData) -> RelativeStrengthIndexResult:
        ...
        series: List[RelativeStrengthIndexIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(RelativeStrengthIndexIndicatorPoint(
                index=i,
        ...
        return RelativeStrengthIndexResult(
            current_value=curr,
