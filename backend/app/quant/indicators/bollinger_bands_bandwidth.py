"""
Bollinger Bands, %B and BandWidth Squeeze Indicator
...
class BollingerBandsBandwidthInputData(BaseModel):
...
class BollingerBandsBandwidthIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class BollingerBandsBandwidthResult(BaseModel):
    indicator_title: str = "Bollinger Bands, %B and BandWidth Squeeze Indicator"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[BollingerBandsBandwidthIndicatorPoint]

class BollingerBandsBandwidthEngine:
    @classmethod
    def calculate(cls, data: BollingerBandsBandwidthInputData) -> BollingerBandsBandwidthResult:
        ...
        series: List[BollingerBandsBandwidthIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(BollingerBandsBandwidthIndicatorPoint(
                index=i,
        ...
        return BollingerBandsBandwidthResult(
            current_value=curr,
