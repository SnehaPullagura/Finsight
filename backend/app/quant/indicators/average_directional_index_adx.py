"""
Welles Wilder Average Directional Index (ADX/DMI)
...
class AverageDirectionalIndexAdxInputData(BaseModel):
...
class AverageDirectionalIndexAdxIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class AverageDirectionalIndexAdxResult(BaseModel):
    indicator_title: str = "Welles Wilder Average Directional Index (ADX/DMI)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[AverageDirectionalIndexAdxIndicatorPoint]

class AverageDirectionalIndexAdxEngine:
    @classmethod
    def calculate(cls, data: AverageDirectionalIndexAdxInputData) -> AverageDirectionalIndexAdxResult:
        ...
        series: List[AverageDirectionalIndexAdxIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(AverageDirectionalIndexAdxIndicatorPoint(
                index=i,
        ...
        return AverageDirectionalIndexAdxResult(
            current_value=curr,
