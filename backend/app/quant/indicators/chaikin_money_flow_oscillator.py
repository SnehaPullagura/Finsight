"""
Chaikin Money Flow (CMF) 20-Period Accumulation
...
class ChaikinMoneyFlowOscillatorInputData(BaseModel):
...
class ChaikinMoneyFlowOscillatorIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class ChaikinMoneyFlowOscillatorResult(BaseModel):
    indicator_title: str = "Chaikin Money Flow (CMF) 20-Period Accumulation"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[ChaikinMoneyFlowOscillatorIndicatorPoint]

class ChaikinMoneyFlowOscillatorEngine:
    @classmethod
    def calculate(cls, data: ChaikinMoneyFlowOscillatorInputData) -> ChaikinMoneyFlowOscillatorResult:
        ...
        series: List[ChaikinMoneyFlowOscillatorIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(ChaikinMoneyFlowOscillatorIndicatorPoint(
                index=i,
        ...
        return ChaikinMoneyFlowOscillatorResult(
            current_value=curr,
