"""
On-Balance Volume (OBV) & Volume Accumulation Trend
...
class OnBalanceVolumeAccumulationInputData(BaseModel):
...
class OnBalanceVolumeAccumulationIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class OnBalanceVolumeAccumulationResult(BaseModel):
    indicator_title: str = "On-Balance Volume (OBV) & Volume Accumulation Trend"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[OnBalanceVolumeAccumulationIndicatorPoint]

class OnBalanceVolumeAccumulationEngine:
    @classmethod
    def calculate(cls, data: OnBalanceVolumeAccumulationInputData) -> OnBalanceVolumeAccumulationResult:
        ...
        series: List[OnBalanceVolumeAccumulationIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(OnBalanceVolumeAccumulationIndicatorPoint(
                index=i,
        ...
        return OnBalanceVolumeAccumulationResult(
            current_value=curr,
