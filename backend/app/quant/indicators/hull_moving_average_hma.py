"""
Alan Hull Moving Average (HMA) Zero-Lag Filter
...
class HullMovingAverageHmaInputData(BaseModel):
...
class HullMovingAverageHmaIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class HullMovingAverageHmaResult(BaseModel):
    indicator_title: str = "Alan Hull Moving Average (HMA) Zero-Lag Filter"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[HullMovingAverageHmaIndicatorPoint]

class HullMovingAverageHmaEngine:
    @classmethod
    def calculate(cls, data: HullMovingAverageHmaInputData) -> HullMovingAverageHmaResult:
        ...
        series: List[HullMovingAverageHmaIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(HullMovingAverageHmaIndicatorPoint(
                index=i,
        ...
        return HullMovingAverageHmaResult(
            current_value=curr,
