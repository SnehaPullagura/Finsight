...
class TrixTripleExponentialOscillatorInputData(BaseModel):
...
class TrixTripleExponentialOscillatorIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class TrixTripleExponentialOscillatorResult(BaseModel):
    indicator_title: str = "TRIX 1-Day ROC of Triple Smoothed EMA"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[TrixTripleExponentialOscillatorIndicatorPoint]

class TrixTripleExponentialOscillatorEngine:
    @classmethod
    def calculate(cls, data: TrixTripleExponentialOscillatorInputData) -> TrixTripleExponentialOscillatorResult:
        ...
        series: List[TrixTripleExponentialOscillatorIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(TrixTripleExponentialOscillatorIndicatorPoint(
                index=i,
        ...
        return TrixTripleExponentialOscillatorResult(
            current_value=curr,
