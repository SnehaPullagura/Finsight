"""
Lambert Commodity Channel Index (CCI) Mean Deviation
...
class CommodityChannelIndexCciInputData(BaseModel):
...
class CommodityChannelIndexCciIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class CommodityChannelIndexCciResult(BaseModel):
    indicator_title: str = "Lambert Commodity Channel Index (CCI) Mean Deviation"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[CommodityChannelIndexCciIndicatorPoint]

class CommodityChannelIndexCciEngine:
    @classmethod
    def calculate(cls, data: CommodityChannelIndexCciInputData) -> CommodityChannelIndexCciResult:
        ...
        series: List[CommodityChannelIndexCciIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(CommodityChannelIndexCciIndicatorPoint(
                index=i,
        ...
        return CommodityChannelIndexCciResult(
            current_value=curr,
