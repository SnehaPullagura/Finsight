...
class AroonOscillatorUpDownInputData(BaseModel):
...
class AroonOscillatorUpDownIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class AroonOscillatorUpDownResult(BaseModel):
    indicator_title: str = "Aroon Indicator (Aroon Up, Aroon Down, Aroon Oscillator)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[AroonOscillatorUpDownIndicatorPoint]

class AroonOscillatorUpDownEngine:
    @classmethod
    def calculate(cls, data: AroonOscillatorUpDownInputData) -> AroonOscillatorUpDownResult:
        ...
        series: List[AroonOscillatorUpDownIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(AroonOscillatorUpDownIndicatorPoint(
                index=i,
        ...
        return AroonOscillatorUpDownResult(
            current_value=curr,
