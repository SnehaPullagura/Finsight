"""
Fast and Slow Stochastic %K / %D Momentum Oscillator
...
class StochasticOscillatorFastSlowInputData(BaseModel):
...
class StochasticOscillatorFastSlowIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class StochasticOscillatorFastSlowResult(BaseModel):
    indicator_title: str = "Fast and Slow Stochastic %K / %D Momentum Oscillator"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[StochasticOscillatorFastSlowIndicatorPoint]

class StochasticOscillatorFastSlowEngine:
    @classmethod
    def calculate(cls, data: StochasticOscillatorFastSlowInputData) -> StochasticOscillatorFastSlowResult:
        ...
        series: List[StochasticOscillatorFastSlowIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(StochasticOscillatorFastSlowIndicatorPoint(
                index=i,
        ...
        return StochasticOscillatorFastSlowResult(
            current_value=curr,
