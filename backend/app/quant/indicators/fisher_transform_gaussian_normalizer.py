"""
Ehlers Fisher Transform Gaussian Return Normalizer
...
class FisherTransformGaussianNormalizerInputData(BaseModel):
...
class FisherTransformGaussianNormalizerIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class FisherTransformGaussianNormalizerResult(BaseModel):
    indicator_title: str = "Ehlers Fisher Transform Gaussian Return Normalizer"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[FisherTransformGaussianNormalizerIndicatorPoint]

class FisherTransformGaussianNormalizerEngine:
    @classmethod
    def calculate(cls, data: FisherTransformGaussianNormalizerInputData) -> FisherTransformGaussianNormalizerResult:
        ...
        series: List[FisherTransformGaussianNormalizerIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(FisherTransformGaussianNormalizerIndicatorPoint(
                index=i,
        ...
        return FisherTransformGaussianNormalizerResult(
            current_value=curr,
