"""
Larry Williams Ultimate Oscillator (7, 14, 28 Days)
...
class UltimateOscillatorMultiTimeframeInputData(BaseModel):
...
class UltimateOscillatorMultiTimeframeIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class UltimateOscillatorMultiTimeframeResult(BaseModel):
    indicator_title: str = "Larry Williams Ultimate Oscillator (7, 14, 28 Days)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[UltimateOscillatorMultiTimeframeIndicatorPoint]

class UltimateOscillatorMultiTimeframeEngine:
    @classmethod
    def calculate(cls, data: UltimateOscillatorMultiTimeframeInputData) -> UltimateOscillatorMultiTimeframeResult:
        ...
        series: List[UltimateOscillatorMultiTimeframeIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(UltimateOscillatorMultiTimeframeIndicatorPoint(
                index=i,
        ...
        return UltimateOscillatorMultiTimeframeResult(
            current_value=curr,
