...
class TrueStrengthIndexTsiInputData(BaseModel):
...
class TrueStrengthIndexTsiIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class TrueStrengthIndexTsiResult(BaseModel):
    indicator_title: str = "True Strength Index (TSI) Double Smoothed Momentum"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[TrueStrengthIndexTsiIndicatorPoint]

class TrueStrengthIndexTsiEngine:
    @classmethod
    def calculate(cls, data: TrueStrengthIndexTsiInputData) -> TrueStrengthIndexTsiResult:
        ...
        series: List[TrueStrengthIndexTsiIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(TrueStrengthIndexTsiIndicatorPoint(
                index=i,
        ...
        return TrueStrengthIndexTsiResult(
            current_value=curr,
