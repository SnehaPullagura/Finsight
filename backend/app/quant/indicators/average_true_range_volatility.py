"""
Average True Range (ATR) & True Range Volatility Metric
...
class AverageTrueRangeVolatilityInputData(BaseModel):
...
class AverageTrueRangeVolatilityIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class AverageTrueRangeVolatilityResult(BaseModel):
    indicator_title: str = "Average True Range (ATR) & True Range Volatility Metric"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[AverageTrueRangeVolatilityIndicatorPoint]

class AverageTrueRangeVolatilityEngine:
    @classmethod
    def calculate(cls, data: AverageTrueRangeVolatilityInputData) -> AverageTrueRangeVolatilityResult:
        ...
        series: List[AverageTrueRangeVolatilityIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(AverageTrueRangeVolatilityIndicatorPoint(
                index=i,
        ...
        return AverageTrueRangeVolatilityResult(
            current_value=curr,
