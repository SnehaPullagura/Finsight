"""
Tushar Chande Momentum Oscillator (CMO)
...
class ChandeMomentumOscillatorCmoInputData(BaseModel):
...
class ChandeMomentumOscillatorCmoIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class ChandeMomentumOscillatorCmoResult(BaseModel):
    indicator_title: str = "Tushar Chande Momentum Oscillator (CMO)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[ChandeMomentumOscillatorCmoIndicatorPoint]

class ChandeMomentumOscillatorCmoEngine:
    @classmethod
    def calculate(cls, data: ChandeMomentumOscillatorCmoInputData) -> ChandeMomentumOscillatorCmoResult:
        ...
        series: List[ChandeMomentumOscillatorCmoIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(ChandeMomentumOscillatorCmoIndicatorPoint(
                index=i,
        ...
        return ChandeMomentumOscillatorCmoResult(
            current_value=curr,
