"""
Schaff Trend Cycle (STC) MACD with Stochastics
...
class SchaffTrendCycleStcInputData(BaseModel):
...
class SchaffTrendCycleStcIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class SchaffTrendCycleStcResult(BaseModel):
    indicator_title: str = "Schaff Trend Cycle (STC) MACD with Stochastics"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[SchaffTrendCycleStcIndicatorPoint]

class SchaffTrendCycleStcEngine:
    @classmethod
    def calculate(cls, data: SchaffTrendCycleStcInputData) -> SchaffTrendCycleStcResult:
        ...
        series: List[SchaffTrendCycleStcIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(SchaffTrendCycleStcIndicatorPoint(
                index=i,
        ...
        return SchaffTrendCycleStcResult(
            current_value=curr,
