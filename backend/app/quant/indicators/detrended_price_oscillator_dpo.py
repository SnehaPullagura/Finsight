"""
Detrended Price Oscillator (DPO) Cycle Identifier
...
class DetrendedPriceOscillatorDpoInputData(BaseModel):
...
class DetrendedPriceOscillatorDpoIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class DetrendedPriceOscillatorDpoResult(BaseModel):
    indicator_title: str = "Detrended Price Oscillator (DPO) Cycle Identifier"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[DetrendedPriceOscillatorDpoIndicatorPoint]

class DetrendedPriceOscillatorDpoEngine:
    @classmethod
    def calculate(cls, data: DetrendedPriceOscillatorDpoInputData) -> DetrendedPriceOscillatorDpoResult:
        ...
        series: List[DetrendedPriceOscillatorDpoIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(DetrendedPriceOscillatorDpoIndicatorPoint(
                index=i,
        ...
        return DetrendedPriceOscillatorDpoResult(
            current_value=curr,
