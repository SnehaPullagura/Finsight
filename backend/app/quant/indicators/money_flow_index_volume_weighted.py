...
class MoneyFlowIndexVolumeWeightedInputData(BaseModel):
...
class MoneyFlowIndexVolumeWeightedIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class MoneyFlowIndexVolumeWeightedResult(BaseModel):
    indicator_title: str = "Money Flow Index (MFI) Volume-Weighted RSI"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[MoneyFlowIndexVolumeWeightedIndicatorPoint]

class MoneyFlowIndexVolumeWeightedEngine:
    @classmethod
    def calculate(cls, data: MoneyFlowIndexVolumeWeightedInputData) -> MoneyFlowIndexVolumeWeightedResult:
        ...
        series: List[MoneyFlowIndexVolumeWeightedIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(MoneyFlowIndexVolumeWeightedIndicatorPoint(
                index=i,
        ...
        return MoneyFlowIndexVolumeWeightedResult(
            current_value=curr,
