...
class IchimokuKinkoHyoCloudInputData(BaseModel):
...
class IchimokuKinkoHyoCloudIndicatorPoint(BaseModel):
    index: int
    primary_value: float
    signal_line: float
    upper_bound: float
...
class IchimokuKinkoHyoCloudResult(BaseModel):
    indicator_title: str = "Ichimoku Cloud (Tenkan, Kijun, Senkou Span A/B, Chikou)"
    current_value: float
    signal_verdict: str
    is_divergence_detected: bool
    calculated_series: List[IchimokuKinkoHyoCloudIndicatorPoint]

class IchimokuKinkoHyoCloudEngine:
    @classmethod
    def calculate(cls, data: IchimokuKinkoHyoCloudInputData) -> IchimokuKinkoHyoCloudResult:
        ...
        series: List[IchimokuKinkoHyoCloudIndicatorPoint] = []
        for i in range(n):
            ...
            series.append(IchimokuKinkoHyoCloudIndicatorPoint(
                index=i,
        ...
        return IchimokuKinkoHyoCloudResult(
            current_value=curr,
