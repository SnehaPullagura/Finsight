"""
Early Supplier Invoice Dynamic Discounting & Sliding Scale APR
...
class SupplyChainDynamicDiscountingConfig(BaseModel):
...
class SupplyChainDynamicDiscountingExecutionEvent(BaseModel):
    event_id: str
    event_timestamp: str
    source_entity: str
...
class SupplyChainDynamicDiscountingSummary(BaseModel):
    module_title: str = "Early Supplier Invoice Dynamic Discounting & Sliding Scale APR"
    ...
    events: List[SupplyChainDynamicDiscountingExecutionEvent]
    action_items: List[str]

class SupplyChainDynamicDiscountingEngine:
    @classmethod
    def process_treasury_cycle(
        cls, accounts_data: List[Dict[str, Any]], config: SupplyChainDynamicDiscountingConfig
    ) -> SupplyChainDynamicDiscountingSummary:
        tot_vol = 0.0
        events = []
        ...
        for idx, acc in enumerate(accounts_data, 1):
            ...
            if bal > config.target_cash_cushion_amount + config.sweep_threshold_tolerance:
                ...
                tot_vol += surplus
                events.append(SupplyChainDynamicDiscountingExecutionEvent(
        ...
        return SupplyChainDynamicDiscountingSummary(
