"""
Employees Provident Fund (EPF) and VPF Growth Accumulation Calculator
...
class EpfCalculatorInputs(BaseModel):
...
class EpfCalculatorYearBreakdown(BaseModel):
    year_number: int
...
class EpfCalculatorResult(BaseModel):
    calculator_name: str = "Employees Provident Fund (EPF) and VPF Growth Accumulation Calculator"
    ...
    yearly_schedule: List[EpfCalculatorYearBreakdown]

class EpfCalculatorEngine:
    @classmethod
    def calculate(cls, inp: EpfCalculatorInputs) -> EpfCalculatorResult:
        ...
        for yr in range(1, years + 1):
            ...
            yearly.append(EpfCalculatorYearBreakdown(
                year_number=yr,
