"""
Fixed Income 2Y-5Y-10Y Butterfly Curve Trade Optimizer
...
class YieldCurveButterflySpreadConfig(BaseModel):
    strategy_identifier: str = "YIELD_CURVE_BUTTERFLY_SPREAD"
...
class YieldCurveButterflySpreadSignal(BaseModel):
...
class YieldCurveButterflySpreadEngine:
    @classmethod
    def generate_signal(
        cls, price_series: List[float], config: YieldCurveButterflySpreadConfig
    ) -> YieldCurveButterflySpreadSignal:
        if not price_series or len(price_series) < config.lookback_periods:
            curr = price_series[-1] if price_series else 24500.0
            return YieldCurveButterflySpreadSignal(
