"""
ATR Supertrend and Exponential Moving Average Crossover
...
class TrendFollowingSupertrendConfig(BaseModel):
    strategy_identifier: str = "TREND_FOLLOWING_SUPERTREND"
...
class TrendFollowingSupertrendSignal(BaseModel):
...
class TrendFollowingSupertrendEngine:
    @classmethod
    def generate_signal(
        cls, price_series: List[float], config: TrendFollowingSupertrendConfig
    ) -> TrendFollowingSupertrendSignal:
        if not price_series or len(price_series) < config.lookback_periods:
            curr = price_series[-1] if price_series else 24500.0
            return TrendFollowingSupertrendSignal(
