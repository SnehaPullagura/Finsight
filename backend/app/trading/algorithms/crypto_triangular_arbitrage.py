"""
Multi-Exchange Cross-Currency Triangular Arbitrage Detector
...
class CryptoTriangularArbitrageConfig(BaseModel):
    strategy_identifier: str = "CRYPTO_TRIANGULAR_ARBITRAGE"
...
class CryptoTriangularArbitrageSignal(BaseModel):
...
class CryptoTriangularArbitrageEngine:
    @classmethod
    def generate_signal(
        cls, price_series: List[float], config: CryptoTriangularArbitrageConfig
    ) -> CryptoTriangularArbitrageSignal:
        if not price_series or len(price_series) < config.lookback_periods:
            curr = price_series[-1] if price_series else 24500.0
            return CryptoTriangularArbitrageSignal(
