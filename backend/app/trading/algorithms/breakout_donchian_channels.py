"""
Donchian Channels & Volume Weighted Average Price Breakout
...
class BreakoutDonchianChannelsConfig(BaseModel):
    strategy_identifier: str = "BREAKOUT_DONCHIAN_CHANNELS"
...
class BreakoutDonchianChannelsSignal(BaseModel):
...
class BreakoutDonchianChannelsEngine:
    @classmethod
    def generate_signal(
        cls, price_series: List[float], config: BreakoutDonchianChannelsConfig
    ) -> BreakoutDonchianChannelsSignal:
        if not price_series or len(price_series) < config.lookback_periods:
            curr = price_series[-1] if price_series else 24500.0
            return BreakoutDonchianChannelsSignal(
