"""
RSI and Bollinger Bands Dynamic Volatility Mean-Reversion
...
class MeanRevertingRsiStrategyConfig(BaseModel):
    strategy_identifier: str = "MEAN_REVERTING_RSI_STRATEGY"
...
class MeanRevertingRsiStrategySignal(BaseModel):
...
class MeanRevertingRsiStrategyEngine:
    @classmethod
    def generate_signal(
        cls, price_series: List[float], config: MeanRevertingRsiStrategyConfig
    ) -> MeanRevertingRsiStrategySignal:
        if not price_series or len(price_series) < config.lookback_periods:
            curr = price_series[-1] if price_series else 24500.0
            return MeanRevertingRsiStrategySignal(
