"""
Adverse Selection & Toxic Flow VP-IN (Volume-Synchronized PIN)
...
class AdverseSelectionToxicFlowFilterParentOrder(BaseModel):
...
class AdverseSelectionToxicFlowFilterChildSlice(BaseModel):
    slice_index: int
    execution_time: str
    venue_name: str
...
class AdverseSelectionToxicFlowFilterExecutionReport(BaseModel):
    algorithm_name: str = "Adverse Selection & Toxic Flow VP-IN (Volume-Synchronized PIN)"
    ...
    child_slices: List[AdverseSelectionToxicFlowFilterChildSlice]
    tca_summary_notes: List[str]

class AdverseSelectionToxicFlowFilterEngine:
    @classmethod
    def execute_routing(
        cls, order: AdverseSelectionToxicFlowFilterParentOrder
    ) -> AdverseSelectionToxicFlowFilterExecutionReport:
        slices = []
        n_slices = 8
        ...
        for i in range(1, n_slices + 1):
            ...
            slices.append(AdverseSelectionToxicFlowFilterChildSlice(
                slice_index=i,
                execution_time=ts,
                venue_name=venue,
        ...
        return AdverseSelectionToxicFlowFilterExecutionReport(
