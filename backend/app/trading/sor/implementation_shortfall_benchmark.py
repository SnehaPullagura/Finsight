"""
Implementation Shortfall (IS) Execution Drift & Slippage Decomposer
...
class ImplementationShortfallBenchmarkParentOrder(BaseModel):
...
class ImplementationShortfallBenchmarkChildSlice(BaseModel):
    slice_index: int
    execution_time: str
    venue_name: str
...
class ImplementationShortfallBenchmarkExecutionReport(BaseModel):
    algorithm_name: str = "Implementation Shortfall (IS) Execution Drift & Slippage Decomposer"
    ...
    child_slices: List[ImplementationShortfallBenchmarkChildSlice]
    tca_summary_notes: List[str]

class ImplementationShortfallBenchmarkEngine:
    @classmethod
    def execute_routing(
        cls, order: ImplementationShortfallBenchmarkParentOrder
    ) -> ImplementationShortfallBenchmarkExecutionReport:
        slices = []
        n_slices = 8
        ...
        for i in range(1, n_slices + 1):
            ...
            slices.append(ImplementationShortfallBenchmarkChildSlice(
                slice_index=i,
                execution_time=ts,
                venue_name=venue,
        ...
        return ImplementationShortfallBenchmarkExecutionReport(
