"""
Almgren-Chriss Optimal Liquidation Trajectory & Impact Model
...
class AlmgrenChrissMarketImpactModelParentOrder(BaseModel):
...
class AlmgrenChrissMarketImpactModelChildSlice(BaseModel):
    slice_index: int
    execution_time: str
    venue_name: str
...
class AlmgrenChrissMarketImpactModelExecutionReport(BaseModel):
    algorithm_name: str = "Almgren-Chriss Optimal Liquidation Trajectory & Impact Model"
    ...
    child_slices: List[AlmgrenChrissMarketImpactModelChildSlice]
    tca_summary_notes: List[str]

class AlmgrenChrissMarketImpactModelEngine:
    @classmethod
    def execute_routing(
        cls, order: AlmgrenChrissMarketImpactModelParentOrder
    ) -> AlmgrenChrissMarketImpactModelExecutionReport:
        slices = []
        n_slices = 8
        ...
        for i in range(1, n_slices + 1):
            ...
            slices.append(AlmgrenChrissMarketImpactModelChildSlice(
                slice_index=i,
                execution_time=ts,
                venue_name=venue,
        ...
        return AlmgrenChrissMarketImpactModelExecutionReport(
