"""
InvIT Net Distributable Cash Flow (NDCF 90% Mandatory Payout)
...
class InvitDistributionYieldNdcfAssetTelemetry(BaseModel):
...
class InvitDistributionYieldNdcfDistributionQuarter(BaseModel):
    quarter_label: str
...
class InvitDistributionYieldNdcfValuationReport(BaseModel):
    asset_name: str = "InvIT Net Distributable Cash Flow (NDCF 90% Mandatory Payout)"
    ...
    quarterly_distribution_schedule: List[InvitDistributionYieldNdcfDistributionQuarter]
    regulatory_compliance_check: List[str]

class InvitDistributionYieldNdcfEngine:
    @classmethod
    def evaluate_asset_cash_flow(
        cls, a: InvitDistributionYieldNdcfAssetTelemetry
    ) -> InvitDistributionYieldNdcfValuationReport:
        ...
        quarters: List[InvitDistributionYieldNdcfDistributionQuarter] = []
        q_dpu = annual_dpu / 4.0

        for q in range(1, 5):
            quarters.append(InvitDistributionYieldNdcfDistributionQuarter(
        ...
        return InvitDistributionYieldNdcfValuationReport(
