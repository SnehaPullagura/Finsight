"""
Michaud Resampled Efficient Frontier Monte Carlo Optimizer
...
class ResampledEfficiencyMichaudMcOptimizationInput(BaseModel):
...
class ResampledEfficiencyMichaudMcAllocatedAsset(BaseModel):
    asset_id: str
    optimal_allocation_weight_pct: float
    marginal_risk_contribution_pct: float
    expected_annual_return_pct: float

class ResampledEfficiencyMichaudMcOptimizationResult(BaseModel):
    algorithm_title: str = "Michaud Resampled Efficient Frontier Monte Carlo Optimizer"
    ...
    optimal_weights: List[ResampledEfficiencyMichaudMcAllocatedAsset]
    mathematical_convergence_notes: List[str]

class ResampledEfficiencyMichaudMcEngine:
    @classmethod
    def run_optimization(
        cls, inp: ResampledEfficiencyMichaudMcOptimizationInput
    ) -> ResampledEfficiencyMichaudMcOptimizationResult:
        ...
        for i in range(n):
            ...
            allocated_items.append(ResampledEfficiencyMichaudMcAllocatedAsset(
        ...
        return ResampledEfficiencyMichaudMcOptimizationResult(
