"""
Rainbow Option on Best-of/Worst-of Basket Correlation Matrix
...
class RainbowMultiAssetWorstOfBasketContractTerms(BaseModel):
...
class RainbowMultiAssetWorstOfBasketObservationStep(BaseModel):
    step_number: int
...
class RainbowMultiAssetWorstOfBasketValuationReport(BaseModel):
    product_title: str = "Rainbow Option on Best-of/Worst-of Basket Correlation Matrix"
    ...
    observation_timeline: List[RainbowMultiAssetWorstOfBasketObservationStep]
    risk_disclosures: List[str]

class RainbowMultiAssetWorstOfBasketEngine:
    @classmethod
    def price_structure(
        cls, c: RainbowMultiAssetWorstOfBasketContractTerms
    ) -> RainbowMultiAssetWorstOfBasketValuationReport:
        n_obs = int(round((c.tenure_years * 12.0) / c.observation_frequency_months))
        today = datetime.date.today()
        
        timeline: List[RainbowMultiAssetWorstOfBasketObservationStep] = []
        ...
        for i in range(1, n_obs + 1):
            ...
            timeline.append(RainbowMultiAssetWorstOfBasketObservationStep(
                step_number=i,
        ...
        return RainbowMultiAssetWorstOfBasketValuationReport(
