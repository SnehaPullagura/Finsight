"""
Target Redemption Note (TARN) Monte Carlo Early Termination Model
...
class TargetRedemptionNoteTarnPricerContractTerms(BaseModel):
...
class TargetRedemptionNoteTarnPricerObservationStep(BaseModel):
    step_number: int
...
class TargetRedemptionNoteTarnPricerValuationReport(BaseModel):
    product_title: str = "Target Redemption Note (TARN) Monte Carlo Early Termination Model"
    ...
    observation_timeline: List[TargetRedemptionNoteTarnPricerObservationStep]
    risk_disclosures: List[str]

class TargetRedemptionNoteTarnPricerEngine:
    @classmethod
    def price_structure(
        cls, c: TargetRedemptionNoteTarnPricerContractTerms
    ) -> TargetRedemptionNoteTarnPricerValuationReport:
        n_obs = int(round((c.tenure_years * 12.0) / c.observation_frequency_months))
        today = datetime.date.today()
        
        timeline: List[TargetRedemptionNoteTarnPricerObservationStep] = []
        ...
        for i in range(1, n_obs + 1):
            ...
            timeline.append(TargetRedemptionNoteTarnPricerObservationStep(
                step_number=i,
        ...
        return TargetRedemptionNoteTarnPricerValuationReport(
