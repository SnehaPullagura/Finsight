"""
Up-and-Out / Down-and-In Barrier Option Closed-Form & MC Engine
...
class BarrierOptionKnockInOutPricerContractTerms(BaseModel):
...
class BarrierOptionKnockInOutPricerObservationStep(BaseModel):
    step_number: int
...
class BarrierOptionKnockInOutPricerValuationReport(BaseModel):
    product_title: str = "Up-and-Out / Down-and-In Barrier Option Closed-Form & MC Engine"
    ...
    observation_timeline: List[BarrierOptionKnockInOutPricerObservationStep]
    risk_disclosures: List[str]

class BarrierOptionKnockInOutPricerEngine:
    @classmethod
    def price_structure(
        cls, c: BarrierOptionKnockInOutPricerContractTerms
    ) -> BarrierOptionKnockInOutPricerValuationReport:
        n_obs = int(round((c.tenure_years * 12.0) / c.observation_frequency_months))
        today = datetime.date.today()
        
        timeline: List[BarrierOptionKnockInOutPricerObservationStep] = []
        ...
        for i in range(1, n_obs + 1):
            ...
            timeline.append(BarrierOptionKnockInOutPricerObservationStep(
                step_number=i,
        ...
        return BarrierOptionKnockInOutPricerValuationReport(
