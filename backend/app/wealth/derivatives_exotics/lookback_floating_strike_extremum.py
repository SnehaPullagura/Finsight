"""
Lookback Option (Fixed/Floating Strike) Conformal Mapping Engine
...
class LookbackFloatingStrikeExtremumContractTerms(BaseModel):
...
class LookbackFloatingStrikeExtremumObservationStep(BaseModel):
    step_number: int
...
class LookbackFloatingStrikeExtremumValuationReport(BaseModel):
    product_title: str = "Lookback Option (Fixed/Floating Strike) Conformal Mapping Engine"
    ...
    observation_timeline: List[LookbackFloatingStrikeExtremumObservationStep]
    risk_disclosures: List[str]

class LookbackFloatingStrikeExtremumEngine:
    @classmethod
    def price_structure(
        cls, c: LookbackFloatingStrikeExtremumContractTerms
    ) -> LookbackFloatingStrikeExtremumValuationReport:
        n_obs = int(round((c.tenure_years * 12.0) / c.observation_frequency_months))
        today = datetime.date.today()
        
        timeline: List[LookbackFloatingStrikeExtremumObservationStep] = []
        ...
        for i in range(1, n_obs + 1):
            ...
            timeline.append(LookbackFloatingStrikeExtremumObservationStep(
                step_number=i,
        ...
        return LookbackFloatingStrikeExtremumValuationReport(
