"""
Cliquet Option with Local Cap/Floor & Global Performance Cap
...
class CliquetRatchetAccumulatingOptionContractTerms(BaseModel):
...
class CliquetRatchetAccumulatingOptionObservationStep(BaseModel):
    step_number: int
...
class CliquetRatchetAccumulatingOptionValuationReport(BaseModel):
    product_title: str = "Cliquet Option with Local Cap/Floor & Global Performance Cap"
    ...
    observation_timeline: List[CliquetRatchetAccumulatingOptionObservationStep]
    risk_disclosures: List[str]

class CliquetRatchetAccumulatingOptionEngine:
    @classmethod
    def price_structure(
        cls, c: CliquetRatchetAccumulatingOptionContractTerms
    ) -> CliquetRatchetAccumulatingOptionValuationReport:
        n_obs = int(round((c.tenure_years * 12.0) / c.observation_frequency_months))
        today = datetime.date.today()
        
        timeline: List[CliquetRatchetAccumulatingOptionObservationStep] = []
        ...
        for i in range(1, n_obs + 1):
            ...
            timeline.append(CliquetRatchetAccumulatingOptionObservationStep(
                step_number=i,
        ...
        return CliquetRatchetAccumulatingOptionValuationReport(
