"""
Life-Cycle Target Date Glidepath & Dynamic Equity Derisking Engine
...
class GoalGlidepathAllocationEngineClientGoal(BaseModel):
...
class GoalGlidepathAllocationEngineAnnualGlidePoint(BaseModel):
    year: int
...
class GoalGlidepathAllocationEngineAdvisoryResult(BaseModel):
    strategy_name: str = "Life-Cycle Target Date Glidepath & Dynamic Equity Derisking Engine"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[GoalGlidepathAllocationEngineAnnualGlidePoint]
    action_plan: List[str]

class GoalGlidepathAllocationEngineEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: GoalGlidepathAllocationEngineClientGoal
    ) -> GoalGlidepathAllocationEngineAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[GoalGlidepathAllocationEngineAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(GoalGlidepathAllocationEngineAnnualGlidePoint(
                year=y,
        ...
        return GoalGlidepathAllocationEngineAdvisoryResult(
            strategy_name="Life-Cycle Target Date Glidepath & Dynamic Equity Derisking Engine",
