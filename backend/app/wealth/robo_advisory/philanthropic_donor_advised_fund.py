"""
Donor-Advised Fund (DAF) Tax Deduction & Appreciated Stock Gifting
...
class PhilanthropicDonorAdvisedFundClientGoal(BaseModel):
...
class PhilanthropicDonorAdvisedFundAnnualGlidePoint(BaseModel):
    year: int
...
class PhilanthropicDonorAdvisedFundAdvisoryResult(BaseModel):
    strategy_name: str = "Donor-Advised Fund (DAF) Tax Deduction & Appreciated Stock Gifting"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[PhilanthropicDonorAdvisedFundAnnualGlidePoint]
    action_plan: List[str]

class PhilanthropicDonorAdvisedFundEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: PhilanthropicDonorAdvisedFundClientGoal
    ) -> PhilanthropicDonorAdvisedFundAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[PhilanthropicDonorAdvisedFundAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(PhilanthropicDonorAdvisedFundAnnualGlidePoint(
                year=y,
        ...
        return PhilanthropicDonorAdvisedFundAdvisoryResult(
            strategy_name="Donor-Advised Fund (DAF) Tax Deduction & Appreciated Stock Gifting",
