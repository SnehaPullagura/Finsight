"""
Multi-Factor Smart Beta (Quality, Momentum, Value, Low-Vol) Allocator
...
class FactorTiltSmartBetaAllocatorClientGoal(BaseModel):
...
class FactorTiltSmartBetaAllocatorAnnualGlidePoint(BaseModel):
    year: int
...
class FactorTiltSmartBetaAllocatorAdvisoryResult(BaseModel):
    strategy_name: str = "Multi-Factor Smart Beta (Quality, Momentum, Value, Low-Vol) Allocator"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[FactorTiltSmartBetaAllocatorAnnualGlidePoint]
    action_plan: List[str]

class FactorTiltSmartBetaAllocatorEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: FactorTiltSmartBetaAllocatorClientGoal
    ) -> FactorTiltSmartBetaAllocatorAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[FactorTiltSmartBetaAllocatorAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(FactorTiltSmartBetaAllocatorAnnualGlidePoint(
                year=y,
        ...
        return FactorTiltSmartBetaAllocatorAdvisoryResult(
            strategy_name="Multi-Factor Smart Beta (Quality, Momentum, Value, Low-Vol) Allocator",
