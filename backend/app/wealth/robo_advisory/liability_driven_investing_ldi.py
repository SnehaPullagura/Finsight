"""
Liability-Driven Investing (LDI) Cash-Flow Matching for Financial Goals
...
class LiabilityDrivenInvestingLdiClientGoal(BaseModel):
...
class LiabilityDrivenInvestingLdiAnnualGlidePoint(BaseModel):
    year: int
...
class LiabilityDrivenInvestingLdiAdvisoryResult(BaseModel):
    strategy_name: str = "Liability-Driven Investing (LDI) Cash-Flow Matching for Financial Goals"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[LiabilityDrivenInvestingLdiAnnualGlidePoint]
    action_plan: List[str]

class LiabilityDrivenInvestingLdiEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: LiabilityDrivenInvestingLdiClientGoal
    ) -> LiabilityDrivenInvestingLdiAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[LiabilityDrivenInvestingLdiAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(LiabilityDrivenInvestingLdiAnnualGlidePoint(
                year=y,
        ...
        return LiabilityDrivenInvestingLdiAdvisoryResult(
            strategy_name="Liability-Driven Investing (LDI) Cash-Flow Matching for Financial Goals",
