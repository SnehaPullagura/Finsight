"""
Guaranteed Lifetime Annuity vs Variable SWP Longevity Risk Engine
...
class AnnuityVsSwpLongevityHedgeClientGoal(BaseModel):
...
class AnnuityVsSwpLongevityHedgeAnnualGlidePoint(BaseModel):
    year: int
...
class AnnuityVsSwpLongevityHedgeAdvisoryResult(BaseModel):
    strategy_name: str = "Guaranteed Lifetime Annuity vs Variable SWP Longevity Risk Engine"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[AnnuityVsSwpLongevityHedgeAnnualGlidePoint]
    action_plan: List[str]

class AnnuityVsSwpLongevityHedgeEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: AnnuityVsSwpLongevityHedgeClientGoal
    ) -> AnnuityVsSwpLongevityHedgeAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[AnnuityVsSwpLongevityHedgeAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(AnnuityVsSwpLongevityHedgeAnnualGlidePoint(
                year=y,
        ...
        return AnnuityVsSwpLongevityHedgeAdvisoryResult(
            strategy_name="Guaranteed Lifetime Annuity vs Variable SWP Longevity Risk Engine",
