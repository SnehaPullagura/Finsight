"""
Tax-Aware Asset Location: Taxable vs Tax-Deferred vs Tax-Exempt Accounts
...
class TaxAwareAssetLocationMatrixClientGoal(BaseModel):
...
class TaxAwareAssetLocationMatrixAnnualGlidePoint(BaseModel):
    year: int
...
class TaxAwareAssetLocationMatrixAdvisoryResult(BaseModel):
    strategy_name: str = "Tax-Aware Asset Location: Taxable vs Tax-Deferred vs Tax-Exempt Accounts"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[TaxAwareAssetLocationMatrixAnnualGlidePoint]
    action_plan: List[str]

class TaxAwareAssetLocationMatrixEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: TaxAwareAssetLocationMatrixClientGoal
    ) -> TaxAwareAssetLocationMatrixAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[TaxAwareAssetLocationMatrixAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(TaxAwareAssetLocationMatrixAnnualGlidePoint(
                year=y,
        ...
        return TaxAwareAssetLocationMatrixAdvisoryResult(
            strategy_name="Tax-Aware Asset Location: Taxable vs Tax-Deferred vs Tax-Exempt Accounts",
