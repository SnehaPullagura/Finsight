"""
Transaction Cost & Capital Gains Tax Aware Portfolio Rebalancing
...
class RebalancingTaxCostTradeoffClientGoal(BaseModel):
...
class RebalancingTaxCostTradeoffAnnualGlidePoint(BaseModel):
    year: int
...
class RebalancingTaxCostTradeoffAdvisoryResult(BaseModel):
    strategy_name: str = "Transaction Cost & Capital Gains Tax Aware Portfolio Rebalancing"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[RebalancingTaxCostTradeoffAnnualGlidePoint]
    action_plan: List[str]

class RebalancingTaxCostTradeoffEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: RebalancingTaxCostTradeoffClientGoal
    ) -> RebalancingTaxCostTradeoffAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[RebalancingTaxCostTradeoffAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(RebalancingTaxCostTradeoffAnnualGlidePoint(
                year=y,
        ...
        return RebalancingTaxCostTradeoffAdvisoryResult(
            strategy_name="Transaction Cost & Capital Gains Tax Aware Portfolio Rebalancing",
