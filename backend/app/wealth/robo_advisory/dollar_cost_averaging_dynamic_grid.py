"""
Volatility-Adjusted Value Averaging (VA) vs Dollar Cost Averaging (DCA)
...
class DollarCostAveragingDynamicGridClientGoal(BaseModel):
...
class DollarCostAveragingDynamicGridAnnualGlidePoint(BaseModel):
    year: int
...
class DollarCostAveragingDynamicGridAdvisoryResult(BaseModel):
    strategy_name: str = "Volatility-Adjusted Value Averaging (VA) vs Dollar Cost Averaging (DCA)"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[DollarCostAveragingDynamicGridAnnualGlidePoint]
    action_plan: List[str]

class DollarCostAveragingDynamicGridEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: DollarCostAveragingDynamicGridClientGoal
    ) -> DollarCostAveragingDynamicGridAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[DollarCostAveragingDynamicGridAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(DollarCostAveragingDynamicGridAnnualGlidePoint(
                year=y,
        ...
        return DollarCostAveragingDynamicGridAdvisoryResult(
            strategy_name="Volatility-Adjusted Value Averaging (VA) vs Dollar Cost Averaging (DCA)",
