"""
Direct Indexing Fractional Shares & Custom ESG Tax Loss Harvester
...
class DirectIndexingTaxLossHarvesterClientGoal(BaseModel):
...
class DirectIndexingTaxLossHarvesterAnnualGlidePoint(BaseModel):
    year: int
...
class DirectIndexingTaxLossHarvesterAdvisoryResult(BaseModel):
    strategy_name: str = "Direct Indexing Fractional Shares & Custom ESG Tax Loss Harvester"
    years_to_target: int
    is_goal_on_track: bool
    recommended_monthly_sip: float
    glidepath_schedule: List[DirectIndexingTaxLossHarvesterAnnualGlidePoint]
    action_plan: List[str]

class DirectIndexingTaxLossHarvesterEngine:
    @classmethod
    def generate_advisory_plan(
        cls, goal: DirectIndexingTaxLossHarvesterClientGoal
    ) -> DirectIndexingTaxLossHarvesterAdvisoryResult:
        current_year = datetime.date.today().year
        target_yr = int(goal.target_date[:4])
        years_left = max(1, target_yr - current_year)

        glidepath: List[DirectIndexingTaxLossHarvesterAnnualGlidePoint] = []
        ...
        for y in range(current_year, target_yr + 1):
            ...
            glidepath.append(DirectIndexingTaxLossHarvesterAnnualGlidePoint(
                year=y,
        ...
        return DirectIndexingTaxLossHarvesterAdvisoryResult(
            strategy_name="Direct Indexing Fractional Shares & Custom ESG Tax Loss Harvester",
