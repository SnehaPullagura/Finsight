"""
Fund Subscription Facility Leverage & Unlevered vs Levered IRR
...
class SubscriptionLineOfCreditIrrFundParameters(BaseModel):
...
class SubscriptionLineOfCreditIrrWaterfallTier(BaseModel):
    tier_number: int
...
class SubscriptionLineOfCreditIrrDistributionSummary(BaseModel):
    fund_name: str = "Fund Subscription Facility Leverage & Unlevered vs Levered IRR"
    ...
    waterfall_tiers: List[SubscriptionLineOfCreditIrrWaterfallTier]
    governance_notes: List[str]

class SubscriptionLineOfCreditIrrEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: SubscriptionLineOfCreditIrrFundParameters
    ) -> SubscriptionLineOfCreditIrrDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[SubscriptionLineOfCreditIrrWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(SubscriptionLineOfCreditIrrWaterfallTier(
            tier_number=1,
        ...
        return SubscriptionLineOfCreditIrrDistributionSummary(
