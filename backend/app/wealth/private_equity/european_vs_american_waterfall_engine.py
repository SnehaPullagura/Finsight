"""
European (Whole Fund) vs American (Deal-by-Deal) Waterfall Engine
...
class EuropeanVsAmericanWaterfallEngineFundParameters(BaseModel):
...
class EuropeanVsAmericanWaterfallEngineWaterfallTier(BaseModel):
    tier_number: int
...
class EuropeanVsAmericanWaterfallEngineDistributionSummary(BaseModel):
    fund_name: str = "European (Whole Fund) vs American (Deal-by-Deal) Waterfall Engine"
    ...
    waterfall_tiers: List[EuropeanVsAmericanWaterfallEngineWaterfallTier]
    governance_notes: List[str]

class EuropeanVsAmericanWaterfallEngineEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: EuropeanVsAmericanWaterfallEngineFundParameters
    ) -> EuropeanVsAmericanWaterfallEngineDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[EuropeanVsAmericanWaterfallEngineWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(EuropeanVsAmericanWaterfallEngineWaterfallTier(
            tier_number=1,
        ...
        return EuropeanVsAmericanWaterfallEngineDistributionSummary(
