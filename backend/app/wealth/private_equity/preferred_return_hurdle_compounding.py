"""
Preferred Return (8% Hurdle Rate) Hard vs Soft Hurdle Compounding
...
class PreferredReturnHurdleCompoundingFundParameters(BaseModel):
...
class PreferredReturnHurdleCompoundingWaterfallTier(BaseModel):
    tier_number: int
...
class PreferredReturnHurdleCompoundingDistributionSummary(BaseModel):
    fund_name: str = "Preferred Return (8% Hurdle Rate) Hard vs Soft Hurdle Compounding"
    ...
    waterfall_tiers: List[PreferredReturnHurdleCompoundingWaterfallTier]
    governance_notes: List[str]

class PreferredReturnHurdleCompoundingEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: PreferredReturnHurdleCompoundingFundParameters
    ) -> PreferredReturnHurdleCompoundingDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[PreferredReturnHurdleCompoundingWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(PreferredReturnHurdleCompoundingWaterfallTier(
            tier_number=1,
        ...
        return PreferredReturnHurdleCompoundingDistributionSummary(
