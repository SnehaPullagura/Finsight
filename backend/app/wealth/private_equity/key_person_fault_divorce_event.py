"""
Key Person Clause Suspension & For-Cause GP Removal Liquidation
...
class KeyPersonFaultDivorceEventFundParameters(BaseModel):
...
class KeyPersonFaultDivorceEventWaterfallTier(BaseModel):
    tier_number: int
...
class KeyPersonFaultDivorceEventDistributionSummary(BaseModel):
    fund_name: str = "Key Person Clause Suspension & For-Cause GP Removal Liquidation"
    ...
    waterfall_tiers: List[KeyPersonFaultDivorceEventWaterfallTier]
    governance_notes: List[str]

class KeyPersonFaultDivorceEventEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: KeyPersonFaultDivorceEventFundParameters
    ) -> KeyPersonFaultDivorceEventDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[KeyPersonFaultDivorceEventWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(KeyPersonFaultDivorceEventWaterfallTier(
            tier_number=1,
        ...
        return KeyPersonFaultDivorceEventDistributionSummary(
