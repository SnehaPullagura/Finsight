"""
80/20 Carried Interest & 100% Full GP Catch-Up Calculation Engine
...
class CarriedInterestCatchupMatrixFundParameters(BaseModel):
...
class CarriedInterestCatchupMatrixWaterfallTier(BaseModel):
    tier_number: int
...
class CarriedInterestCatchupMatrixDistributionSummary(BaseModel):
    fund_name: str = "80/20 Carried Interest & 100% Full GP Catch-Up Calculation Engine"
    ...
    waterfall_tiers: List[CarriedInterestCatchupMatrixWaterfallTier]
    governance_notes: List[str]

class CarriedInterestCatchupMatrixEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: CarriedInterestCatchupMatrixFundParameters
    ) -> CarriedInterestCatchupMatrixDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[CarriedInterestCatchupMatrixWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(CarriedInterestCatchupMatrixWaterfallTier(
            tier_number=1,
        ...
        return CarriedInterestCatchupMatrixDistributionSummary(
