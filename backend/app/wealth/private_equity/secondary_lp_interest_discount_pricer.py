"""
Secondary Market LP Interest NAV Discount & Pricing Engine
...
class SecondaryLpInterestDiscountPricerFundParameters(BaseModel):
...
class SecondaryLpInterestDiscountPricerWaterfallTier(BaseModel):
    tier_number: int
...
class SecondaryLpInterestDiscountPricerDistributionSummary(BaseModel):
    fund_name: str = "Secondary Market LP Interest NAV Discount & Pricing Engine"
    ...
    waterfall_tiers: List[SecondaryLpInterestDiscountPricerWaterfallTier]
    governance_notes: List[str]

class SecondaryLpInterestDiscountPricerEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: SecondaryLpInterestDiscountPricerFundParameters
    ) -> SecondaryLpInterestDiscountPricerDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[SecondaryLpInterestDiscountPricerWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(SecondaryLpInterestDiscountPricerWaterfallTier(
            tier_number=1,
        ...
        return SecondaryLpInterestDiscountPricerDistributionSummary(
