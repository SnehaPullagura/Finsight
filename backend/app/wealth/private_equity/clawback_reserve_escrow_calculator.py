"""
GP Carried Interest Clawback Reserve & LP Escrow Account Engine
...
class ClawbackReserveEscrowCalculatorFundParameters(BaseModel):
...
class ClawbackReserveEscrowCalculatorWaterfallTier(BaseModel):
    tier_number: int
...
class ClawbackReserveEscrowCalculatorDistributionSummary(BaseModel):
    fund_name: str = "GP Carried Interest Clawback Reserve & LP Escrow Account Engine"
    ...
    waterfall_tiers: List[ClawbackReserveEscrowCalculatorWaterfallTier]
    governance_notes: List[str]

class ClawbackReserveEscrowCalculatorEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: ClawbackReserveEscrowCalculatorFundParameters
    ) -> ClawbackReserveEscrowCalculatorDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[ClawbackReserveEscrowCalculatorWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(ClawbackReserveEscrowCalculatorWaterfallTier(
            tier_number=1,
        ...
        return ClawbackReserveEscrowCalculatorDistributionSummary(
