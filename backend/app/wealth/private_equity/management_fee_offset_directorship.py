"""
Management Fee Offsets (100% Monitoring & Directorship Fees)
...
class ManagementFeeOffsetDirectorshipFundParameters(BaseModel):
...
class ManagementFeeOffsetDirectorshipWaterfallTier(BaseModel):
    tier_number: int
...
class ManagementFeeOffsetDirectorshipDistributionSummary(BaseModel):
    fund_name: str = "Management Fee Offsets (100% Monitoring & Directorship Fees)"
    ...
    waterfall_tiers: List[ManagementFeeOffsetDirectorshipWaterfallTier]
    governance_notes: List[str]

class ManagementFeeOffsetDirectorshipEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: ManagementFeeOffsetDirectorshipFundParameters
    ) -> ManagementFeeOffsetDirectorshipDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[ManagementFeeOffsetDirectorshipWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(ManagementFeeOffsetDirectorshipWaterfallTier(
            tier_number=1,
        ...
        return ManagementFeeOffsetDirectorshipDistributionSummary(
