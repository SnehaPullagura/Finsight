"""
GP-Led Continuation Fund Rollover Valuation & Fairness Opinion
...
class ContinuationFundGpLedValuationFundParameters(BaseModel):
...
class ContinuationFundGpLedValuationWaterfallTier(BaseModel):
    tier_number: int
...
class ContinuationFundGpLedValuationDistributionSummary(BaseModel):
    fund_name: str = "GP-Led Continuation Fund Rollover Valuation & Fairness Opinion"
    ...
    waterfall_tiers: List[ContinuationFundGpLedValuationWaterfallTier]
    governance_notes: List[str]

class ContinuationFundGpLedValuationEngine:
    @classmethod
    def calculate_waterfall(
        cls, fund: ContinuationFundGpLedValuationFundParameters
    ) -> ContinuationFundGpLedValuationDistributionSummary:
        cash = fund.realized_gross_proceeds
        tiers: List[ContinuationFundGpLedValuationWaterfallTier] = []
        
        # Tier 1: Return of Capital (100% to LP until Drawn Capital repaid)
        tier1_lp = min(cash, fund.drawn_capital_called)
        cash -= tier1_lp
        tiers.append(ContinuationFundGpLedValuationWaterfallTier(
            tier_number=1,
        ...
        return ContinuationFundGpLedValuationDistributionSummary(
