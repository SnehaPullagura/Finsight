"""
Single Family Office (SFO) SPV Investment & Governance Structure
...
class FamilyOfficeInvestmentSpvTrustStructure(BaseModel):
...
class FamilyOfficeInvestmentSpvBeneficiaryShare(BaseModel):
    beneficiary_id: str
    relationship: str
...
class FamilyOfficeInvestmentSpvEstatePlanResult(BaseModel):
    structure_title: str = "Single Family Office (SFO) SPV Investment & Governance Structure"
    ...
    beneficiary_allocations: List[FamilyOfficeInvestmentSpvBeneficiaryShare]
    statutory_governance_clauses: List[str]

class FamilyOfficeInvestmentSpvEngine:
    @classmethod
    def generate_estate_structure(
        cls, t: FamilyOfficeInvestmentSpvTrustStructure
    ) -> FamilyOfficeInvestmentSpvEstatePlanResult:
        ...
        for i in range(1, t.beneficiaries_count + 1):
            shares.append(FamilyOfficeInvestmentSpvBeneficiaryShare(
        ...
        return FamilyOfficeInvestmentSpvEstatePlanResult(
