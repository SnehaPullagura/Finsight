"""
Charitable Remainder Unitrust (CRUT) Tax Deduction & Annuity
...
class CharitableRemainderUnitrustCrutTrustStructure(BaseModel):
...
class CharitableRemainderUnitrustCrutBeneficiaryShare(BaseModel):
    beneficiary_id: str
    relationship: str
...
class CharitableRemainderUnitrustCrutEstatePlanResult(BaseModel):
    structure_title: str = "Charitable Remainder Unitrust (CRUT) Tax Deduction & Annuity"
    ...
    beneficiary_allocations: List[CharitableRemainderUnitrustCrutBeneficiaryShare]
    statutory_governance_clauses: List[str]

class CharitableRemainderUnitrustCrutEngine:
    @classmethod
    def generate_estate_structure(
        cls, t: CharitableRemainderUnitrustCrutTrustStructure
    ) -> CharitableRemainderUnitrustCrutEstatePlanResult:
        ...
        for i in range(1, t.beneficiaries_count + 1):
            shares.append(CharitableRemainderUnitrustCrutBeneficiaryShare(
        ...
        return CharitableRemainderUnitrustCrutEstatePlanResult(
