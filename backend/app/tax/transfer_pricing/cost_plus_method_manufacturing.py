"""
Cost Plus Method (CPM) Contract Manufacturing Markup Engine
...
class CostPlusMethodManufacturingTransactionInput(BaseModel):
...
class CostPlusMethodManufacturingBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class CostPlusMethodManufacturingDeterminationResult(BaseModel):
    methodology_name: str = "Cost Plus Method (CPM) Contract Manufacturing Markup Engine"
    ...
    comparable_set: List[CostPlusMethodManufacturingBenchmarkItem]
    statutory_citations: List[str]

class CostPlusMethodManufacturingEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: CostPlusMethodManufacturingTransactionInput
    ) -> CostPlusMethodManufacturingDeterminationResult:
        ...
        comparables = [
            CostPlusMethodManufacturingBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            CostPlusMethodManufacturingBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            CostPlusMethodManufacturingBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            CostPlusMethodManufacturingBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return CostPlusMethodManufacturingDeterminationResult(
