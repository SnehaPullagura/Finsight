"""
BEPS Master File & Local File Documentation Generator
...
class MasterFileLocalFileGeneratorTransactionInput(BaseModel):
...
class MasterFileLocalFileGeneratorBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class MasterFileLocalFileGeneratorDeterminationResult(BaseModel):
    methodology_name: str = "BEPS Master File & Local File Documentation Generator"
    ...
    comparable_set: List[MasterFileLocalFileGeneratorBenchmarkItem]
    statutory_citations: List[str]

class MasterFileLocalFileGeneratorEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: MasterFileLocalFileGeneratorTransactionInput
    ) -> MasterFileLocalFileGeneratorDeterminationResult:
        ...
        comparables = [
            MasterFileLocalFileGeneratorBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            MasterFileLocalFileGeneratorBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            MasterFileLocalFileGeneratorBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            MasterFileLocalFileGeneratorBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return MasterFileLocalFileGeneratorDeterminationResult(
