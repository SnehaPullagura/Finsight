"""
Resale Price Method (RPM) Routine Distributor Gross Margin
...
class ResalePriceMethodDistributionTransactionInput(BaseModel):
...
class ResalePriceMethodDistributionBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class ResalePriceMethodDistributionDeterminationResult(BaseModel):
    methodology_name: str = "Resale Price Method (RPM) Routine Distributor Gross Margin"
    ...
    comparable_set: List[ResalePriceMethodDistributionBenchmarkItem]
    statutory_citations: List[str]

class ResalePriceMethodDistributionEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: ResalePriceMethodDistributionTransactionInput
    ) -> ResalePriceMethodDistributionDeterminationResult:
        ...
        comparables = [
            ResalePriceMethodDistributionBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            ResalePriceMethodDistributionBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            ResalePriceMethodDistributionBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            ResalePriceMethodDistributionBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return ResalePriceMethodDistributionDeterminationResult(
