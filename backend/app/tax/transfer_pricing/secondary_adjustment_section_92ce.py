"""
Section 92CE Secondary Transfer Pricing Adjustment & Deemed Loan
...
class SecondaryAdjustmentSection92CeTransactionInput(BaseModel):
...
class SecondaryAdjustmentSection92CeBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class SecondaryAdjustmentSection92CeDeterminationResult(BaseModel):
    methodology_name: str = "Section 92CE Secondary Transfer Pricing Adjustment & Deemed Loan"
    ...
    comparable_set: List[SecondaryAdjustmentSection92CeBenchmarkItem]
    statutory_citations: List[str]

class SecondaryAdjustmentSection92CeEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: SecondaryAdjustmentSection92CeTransactionInput
    ) -> SecondaryAdjustmentSection92CeDeterminationResult:
        ...
        comparables = [
            SecondaryAdjustmentSection92CeBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            SecondaryAdjustmentSection92CeBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            SecondaryAdjustmentSection92CeBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            SecondaryAdjustmentSection92CeBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return SecondaryAdjustmentSection92CeDeterminationResult(
