"""
Residual Profit Split Method (RPSM) for High-Value Intangibles
...
class ProfitSplitIntangibleAssetsTransactionInput(BaseModel):
...
class ProfitSplitIntangibleAssetsBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class ProfitSplitIntangibleAssetsDeterminationResult(BaseModel):
    methodology_name: str = "Residual Profit Split Method (RPSM) for High-Value Intangibles"
    ...
    comparable_set: List[ProfitSplitIntangibleAssetsBenchmarkItem]
    statutory_citations: List[str]

class ProfitSplitIntangibleAssetsEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: ProfitSplitIntangibleAssetsTransactionInput
    ) -> ProfitSplitIntangibleAssetsDeterminationResult:
        ...
        comparables = [
            ProfitSplitIntangibleAssetsBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            ProfitSplitIntangibleAssetsBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            ProfitSplitIntangibleAssetsBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            ProfitSplitIntangibleAssetsBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return ProfitSplitIntangibleAssetsDeterminationResult(
