"""
Comparable Uncontrolled Price (CUP) Internal/External Benchmark
...
class ComparableUncontrolledPriceCupTransactionInput(BaseModel):
...
class ComparableUncontrolledPriceCupBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class ComparableUncontrolledPriceCupDeterminationResult(BaseModel):
    methodology_name: str = "Comparable Uncontrolled Price (CUP) Internal/External Benchmark"
    ...
    comparable_set: List[ComparableUncontrolledPriceCupBenchmarkItem]
    statutory_citations: List[str]

class ComparableUncontrolledPriceCupEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: ComparableUncontrolledPriceCupTransactionInput
    ) -> ComparableUncontrolledPriceCupDeterminationResult:
        ...
        comparables = [
            ComparableUncontrolledPriceCupBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            ComparableUncontrolledPriceCupBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            ComparableUncontrolledPriceCupBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            ComparableUncontrolledPriceCupBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return ComparableUncontrolledPriceCupDeterminationResult(
