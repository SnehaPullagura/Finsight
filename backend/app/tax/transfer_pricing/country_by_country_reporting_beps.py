"""
OECD Action 13 Country-by-Country (CbCR) Aggregation Matrix
...
class CountryByCountryReportingBepsTransactionInput(BaseModel):
...
class CountryByCountryReportingBepsBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class CountryByCountryReportingBepsDeterminationResult(BaseModel):
    methodology_name: str = "OECD Action 13 Country-by-Country (CbCR) Aggregation Matrix"
    ...
    comparable_set: List[CountryByCountryReportingBepsBenchmarkItem]
    statutory_citations: List[str]

class CountryByCountryReportingBepsEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: CountryByCountryReportingBepsTransactionInput
    ) -> CountryByCountryReportingBepsDeterminationResult:
        ...
        comparables = [
            CountryByCountryReportingBepsBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            CountryByCountryReportingBepsBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            CountryByCountryReportingBepsBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            CountryByCountryReportingBepsBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return CountryByCountryReportingBepsDeterminationResult(
