"""
Transactional Net Margin Method (TNMM) Operating Profit Markup
...
class TransactionalNetMarginMethodTransactionInput(BaseModel):
...
class TransactionalNetMarginMethodBenchmarkItem(BaseModel):
    company_name: str
    operating_profit_margin_pct: float
    is_in_arms_length_range: bool

class TransactionalNetMarginMethodDeterminationResult(BaseModel):
    methodology_name: str = "Transactional Net Margin Method (TNMM) Operating Profit Markup"
    ...
    comparable_set: List[TransactionalNetMarginMethodBenchmarkItem]
    statutory_citations: List[str]

class TransactionalNetMarginMethodEngine:
    @classmethod
    def evaluate_arms_length(
        cls, inp: TransactionalNetMarginMethodTransactionInput
    ) -> TransactionalNetMarginMethodDeterminationResult:
        ...
        comparables = [
            TransactionalNetMarginMethodBenchmarkItem(company_name="TechServices Alpha Ltd", operating_profit_margin_pct=15.4, is_in_arms_length_range=True),
            TransactionalNetMarginMethodBenchmarkItem(company_name="DataSolutions Beta Ltd", operating_profit_margin_pct=17.2, is_in_arms_length_range=True),
            TransactionalNetMarginMethodBenchmarkItem(company_name="Global Systems Gamma Ltd", operating_profit_margin_pct=18.1, is_in_arms_length_range=True),
            TransactionalNetMarginMethodBenchmarkItem(company_name="Software Dynamics Delta Ltd", operating_profit_margin_pct=14.8, is_in_arms_length_range=True)
        ]

        return TransactionalNetMarginMethodDeterminationResult(
